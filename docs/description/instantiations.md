...
Within `<formAvailable>`, a wide range of information can be provided about the analog, digital or hybrid object, such as access conditions (`<accessConditions>`), history of ownership or custody (`<custodHist>`), physical descriptions (`<physicalDescription>`), technical requirements (`<physicalOrTechnicalRequirements>`), relations (`<relations>`), or separated material (`<separatedMaterial>`).
